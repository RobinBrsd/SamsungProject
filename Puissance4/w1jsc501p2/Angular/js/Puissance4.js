jQuery.fn.p4 = function($col, $row, $colorP1, $colorP2, $colorBG, $colorGrid){

    class Puissance4 {

        constructor($paramA, $paramB, $paramC, $paramD) {
            
            if($("#game").val() != undefined){
                $("#game").remove();
                $("#undoP1").remove();
                $("#undoP2").remove();
                $("#area1").remove();
            ...
            if(this.colorP1 != this.colorP2){
                ...
                $("body").append("<h1 id='color-error'> Color are the same ! </h1>");
        ...
        generateGrid($col, $row, $colorP1, $colorP2){
            ...
            game.css("background-color", this.colorGrid);
            $("body").css("background-color", this.colorBG);
            $("html").css("background-color", this.colorBG);
            ...
            if($("#area2").val() == undefined){
                ...
                $("#board").append("<button id='undoP1' type='button'> Undo Last Turn <span class='draw'> &#8635; </span> </button>");
                $("#board").append("<button id='undoP2' type='button'> Undo Last Turn <span class='draw'> &#8635; </span> </button>");
                $("#board").append("<button id='turnP1'> Your Turn </button>");
                $("#board").append("<button id='turnP2' class='hideR'> Your Turn </button>");
                $("#turnP1").css("background-color", this.colorP1);
                $("#turnP2").css("background-color", this.colorP2);
                $("#undoP1").css("background-color", this.colorP1);
                $("#undoP2").css("background-color", this.colorP2);
            }
                                ...
                                if($(spot).hasClass("empty")){
                                    ...
                                    btnUndo1.click(function(){
                                        if($(lastCasePlay).hasClass(that.colorP1 + "playerSpot")){
                                            ...
                                            $(lastCasePlay).children("i").remove();
                                            $(lastCasePlay).append("<i class='fab fa-bitcoin'></i>");
                                            $(lastCasePlay).append("<i class='fab fa-bitcoin drop'></i>");
                                            $(lastCasePlay).addClass("empty");
                                            that.playerTurn = that.colorP1;
                                            $("#turnP2").addClass("hideR");
                                            $("#turnP1").removeClass("hide");
                                            return false;
                                         }
                                     });
                                    ...
                                    btnUndo2.click(function(){
                                        if($(lastCasePlay).hasClass(that.colorP2 + "playerSpot")){
                                            ...
                                            $("#turnP1").addClass("hide");
                                            $("#turnP2").removeClass("hideR");
                                            return false;                                      
                                        }
                                    });   
